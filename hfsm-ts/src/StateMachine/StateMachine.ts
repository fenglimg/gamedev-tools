namespace UnityHFSM {
    import ITriggerable = UnityHFSM.ITriggerable;
    import IStateMachine = UnityHFSM.IStateMachine;
    import IActionable = UnityHFSM.IActionable;
    import ITransitionListener = UnityHFSM.ITransitionListener;
    import StateBase = UnityHFSM.StateBase;
    import TransitionBase = UnityHFSM.TransitionBase;

    export class StateMachine<TOwnId, TStateId, TEvent> extends
        StateBase<TOwnId>,
        ITriggerable<TEvent>,
        ...
        private StateBundle = class <TStateId, TEvent> {
            // 默认情况下，这些字段都是null，只有在需要时才会获取值。
            // => 延迟评估 => 内存效率高，当您只需要功能的子集时
            public state: StateBase<TStateId> | null = null;
            public transitions: Array<TransitionBase<TStateId>> | null = null;
            public triggerToTransitions: Map<TEvent, Array<TransitionBase<TStateId>>> | null = null;

            public addTransition(t: TransitionBase<TStateId>): void {
                if (this.transitions === null) {
                    this.transitions = new Array<TransitionBase<TStateId>>();
                }
                this.transitions.push(t);
            }

            public addTriggerTransition(trigger: TEvent, transition: TransitionBase<TStateId>): void {
                if (this.triggerToTransitions === null) {
                    this.triggerToTransitions = new Map<TEvent, Array<TransitionBase<TStateId>>>();
                }

                let transitionsOfTrigger: Array<TransitionBase<TStateId>>;

                if (!this.triggerToTransitions.has(trigger)) {
                    transitionsOfTrigger = new Array<TransitionBase<TStateId>>();
                } else {
                    // 处理可能为undefined的情况
                    transitionsOfTrigger = this.triggerToTransitions.get(trigger)!;
                    // 使用非空断言(!)因为我们已经用has()检查了是否存在
                }
        ...
        /// </remarks>
        private PendingTransition = class {
            ...
            public listener: ITransitionListener;

            public targetState: TStateId;
            ...
            public SetToExit(listener: ITransitionListener) {
            ...
            public SetToState(target: TStateId, listener: ITransitionListener) {
                this.listener = listener;
        ...
        /** 使用时需要强制转换 Array<TransitionBase<TStateId>>  */
        private static readonly noTransitions: Array<TransitionBase<any>> = [];
        /** 使用时需要强制转换 Map<TEvent, Array<TransitionBase<TStateId>>>  */
        private static readonly noTriggerTransitions: Map<any, Array<TransitionBase<any>>> = new Map();
